function buildSystemPrompt({ mode = 'default', personaText } = {}) {
  ...
  if (mode === 'central_decide') {
    return [
      ...
      '- {"type":"skill_ref", "name":"硬编码稳定技能名", "args":{...}}  — 优先使用稳定中层技能',
      '- {"type":"wait", "timeoutMs":毫秒数}',
      '- {"type":"skill", "skillName":"名", "code":"module.exports.run = async ({api,ctx}) => {...}"}',
      '',
      '## 常用动作链模式',
      '默认优先（稳定技能）: [skill_ref ...]，其次才是低层组合动作',
      '砍树：[navigate nearest_oak_log] → [dig oak_log]（可重复多次）',
      '砍树(稳定): [skill_ref approach_target args={target:"oak_log",sprint:true}] → [skill_ref mine_named_block args={block:"oak_log"}]',
      ...
      '使用稳定技能：[skill_ref name=place_torch_safely args={count:1}]',
