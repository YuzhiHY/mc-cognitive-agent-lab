...
function buildIntentAwareChain({ goalText, playerTexts = [], snapshot = null }) {
  ...
  if (/eat|food|hungry|bread|吃饱|吃|饥饿/.test(b)) {
  ...
  if (/craft|make|合成|制作|造个/.test(b)) {
    let item = 'stick'
    const m = b.match(/\b(wooden_pickaxe|stone_pickaxe|iron_pickaxe|crafting_table|torch|stick|furnace|oak_planks)\b/)
    if (m) item = m[1]
